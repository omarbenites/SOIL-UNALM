---
title: 'Procesamiento de la metadata'
subtitle: '2007'
author: 'Omar Benites'
...
ldpto <- foreach(i=1:length(archivos)) %dopar% {
  out <- get_departamento(archivos[i])
  out
}
names(ldpto) <- basename(archivos)
```

## Combinar las tablas de nutrientes

```{r echo=FALSE, message=FALSE, warning=FALSE}
dtdpto <- data.table::rbindlist(ldpto,fill = TRUE)
write.csv(dtdpto, "data/output/dtdpto.csv",row.names = FALSE)
```
